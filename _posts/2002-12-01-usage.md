...
Windows batch files are text files that contain a series of commands
that the Command Prompt can execute, as if you'd entered them
manually. These are written by writing commands in a text file, but
saving it with (or renaming an existing text file to have) a .bat file
extension.

`databaker` can be used in a batch file.
